...
by_month <- evictions %>%
  group_by(month = floor_date(as.Date(File.Date, format = "%m/%d/%y"), unit = "month")) %>%
  count()

# Store plot in a variable
by_month_plot <- ggplot(by_month) +
  geom_line(mapping = aes(x = month, y = n), size = .1, color = "blue") +
  labs(title = "Number of evictions each month in San Francisco", 
...
evictions_plot <- base_plot + geom_point(mapping = aes(x = long, y = lat), color = "red", alpha = .3) +
  labs(title = "Evictions in San Francisco, 2017") + theme(plot.margin = margin(.3, 0, 0, 0, "cm")) 
